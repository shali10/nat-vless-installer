...
case "$PKG_MGR" in
    ...
    apt)
        ...
        apt-get install -y -qq curl wget tar openssl ca-certificates iptables ip6tables socat >/dev/null 2>&1
        # Debian/Ubuntu 通常用 nftables 代替 iptables
        apt-get install -y -qq iptables >/dev/null 2>&1 || true
        ;;
    dnf|yum)
        $PKG_MGR install -y -q curl wget tar openssl ca-certificates iptables iptables-services socat >/dev/null 2>&1 || true
        ;;
esac
